...
/* CyberRealm: si le compositeur du jeu est actif, redirige les apps lancées
 * par dwl vers lui (WAYLAND_DISPLAY = socket du jeu, DISPLAY = son
 * xwayland-satellite) au lieu du socket de dwl. Sinon, ne change rien. */
static void
cyberrealm_spawn_env(void)
{
	const char *rt;
	char path[4096];
	...
	if (!rt)
		return;
	if (snprintf(path, sizeof(path), "%s/cyberrealm-0", rt) >= (int)sizeof(path))
		return;
	if (access(path, F_OK) != 0)
		return;
	setenv("WAYLAND_DISPLAY", "cyberrealm-0", 1);
	setenv("DISPLAY", ":1", 1);
	setenv("GDK_BACKEND", "wayland", 1);
	setenv("QT_QPA_PLATFORM", "wayland", 1);
	setenv("MOZ_ENABLE_WAYLAND", "1", 1);

	/* Propager l'adresse du bus D-Bus session privé du jeu. Le jeu écrit
	 * cette adresse dans $XDG_RUNTIME_DIR/cyberrealm-session-bus au
	 * démarrage. Sans ça, les apps utilisent le bus D-Bus système (KDE)
	 * et ne trouvent pas les portails CyberRealm → OBS ne voit pas les
	 * sources screencast. */
	if (snprintf(path, sizeof(path), "%s/cyberrealm-session-bus", rt) < (int)sizeof(path)) {
		FILE *f = fopen(path, "r");
		if (f) {
			char dbus_addr[2048] = {0};
			if (fgets(dbus_addr, sizeof(dbus_addr), f)) {
				/* Supprimer le trailing newline */
				size_t len = strlen(dbus_addr);
				while (len > 0 && (dbus_addr[len-1] == '\n' || dbus_addr[len-1] == '\r'))
					dbus_addr[--len] = '\0';
				if (len > 0)
