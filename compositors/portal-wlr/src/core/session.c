...
struct xdpw_session *xdpw_session_create(struct xdpw_state *state, sd_bus *bus, char *object_path) {
	...
	int vtable_ret = sd_bus_add_object_vtable(bus, &sess->slot, object_path, interface_name,
			session_vtable, sess);
	if (vtable_ret < 0) {
		free(sess);
		logprint(ERROR, "dbus: sd_bus_add_object_vtable failed (path=%s): %d (%s)",
			object_path, vtable_ret, strerror(-vtable_ret));
		return NULL;
	}
...
void xdpw_session_destroy(struct xdpw_session *sess) {
	...
	free(sess->screencast_data.capture_claim);
	free(sess);
}
