...
// Taille du meta curseur telle que OBS (plugins/linux-pipewire/pipewire.c)
// la calcule : en-tête spa_meta_cursor + spa_meta_bitmap + pixels ARGB
...
void xdpw_pwr_enqueue_buffer(struct xdpw_screencast_instance *cast) {
	...
	// Curseur METADATA : position/hotspot de la session curseur + image du
	// curseur capturée séparément (xdpw_cursor_state). OBS le lit via
	// spa_meta_cursor_is_valid (id != 0) puis le spa_meta_bitmap à l'offset.
	struct spa_meta_cursor *cursor_meta;
	if ((cursor_meta = spa_buffer_find_meta_data(spa_buf, SPA_META_Cursor, sizeof(*cursor_meta)))) {
		struct xdpw_cursor_state *cs = cast->cursor;
		if (cast->target->with_cursor_meta && cs && cs->entered && cs->image_valid
				&& cs->buffer_allocated && cs->pixels) {
			cursor_meta->id = 1;
			cursor_meta->flags = 0;
			cursor_meta->position.x = cs->x;
			cursor_meta->position.y = cs->y;
			cursor_meta->hotspot.x = cs->hotspot_x;
			cursor_meta->hotspot.y = cs->hotspot_y;
			cursor_meta->bitmap_offset = sizeof(*cursor_meta);

			struct spa_meta_bitmap *bitmap = SPA_MEMBER(cursor_meta,
					cursor_meta->bitmap_offset, struct spa_meta_bitmap);
			bitmap->format = SPA_VIDEO_FORMAT_BGRA;
			bitmap->size = SPA_RECTANGLE(cs->width, cs->height);
			bitmap->stride = cs->stride;
			bitmap->offset = sizeof(*bitmap);
			memcpy(SPA_MEMBER(bitmap, bitmap->offset, void),
					cs->pixels, cs->size);
			logprint(TRACE, "pipewire: cursor meta pos %d,%d size %ux%u",
...
fixate_format:
	...
	if (cast->target->with_cursor_meta) {
		// Taille dérivée de l'image du curseur (déjà connue : contraintes de
		// la session curseur reçues pendant xdpw_ext_ic_session_init), avec
		// une marge jusqu'à 1024x1024 comme côté OBS.
		uint32_t cursor_w = cast->cursor && cast->cursor->buffer_allocated
			? cast->cursor->width : 64;
		uint32_t cursor_h = cast->cursor && cast->cursor->buffer_allocated
			? cast->cursor->height : 64;
		add_pod(&params, spa_pod_builder_add_object(&builder.b,
			SPA_TYPE_OBJECT_ParamMeta, SPA_PARAM_Meta,
			SPA_PARAM_META_type, SPA_POD_Id(SPA_META_Cursor),
			SPA_PARAM_META_size, SPA_POD_CHOICE_RANGE_Int(
				CURSOR_META_SIZE(cursor_w, cursor_h),
