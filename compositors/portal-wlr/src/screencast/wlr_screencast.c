...
int xdpw_wlr_session_init(struct xdpw_screencast_instance *cast) {
	// Le protocole ext-image-capture est préféré quand il est disponible.
	// La condition "output" seule du code amont est trop stricte : un
	// compositeur qui expose ext_image_copy_capture_manager + le manager de
	// source fenêtre (ext_foreign_toplevel_image_capture_source_manager) mais
	// pas celui de sortie (ex. le compositeur de CyberRealm) retomberait sur
	// wlr-screencopy, incapable de capturer des fenêtres (déréférencement de
	// target->output->output sur une cible fenêtre). ext_register_session_cb()
	// vérifie déjà la présence du bon manager par type de cible.
	if (cast->ctx->ext_image_copy_capture_manager
			&& (cast->ctx->ext_output_image_capture_source_manager
